...
fn main() {
    if let Err(error) = run() {
        ...
        if message.contains("`eidolon --help`") {
            eprintln!("error: {message}");
        } else {
            eprintln!(
                "error: {message}

Run `eidolon --help` for usage."
            );
        }
        std::process::exit(1);
    }
...
fn bare_slash_command_guidance(command_name: &str) -> Option<String> {
    ...
    let guidance = if slash_command.resume_supported {
        format!(
            "`eidolon {command_name}` is a slash command. Use `eidolon --resume SESSION.jsonl /{command_name}` or start `eidolon` and run `/{command_name}`."
        )
    } else {
        format!(
            "`eidolon {command_name}` is a slash command. Start `eidolon` and run `/{command_name}` inside the REPL."
        )
    };
    Some(guidance)
}
...
fn parse_direct_slash_cli_action(
    ...
    match SlashCommand::parse(&raw) {
        ...
        Ok(Some(command)) => Err({
            let _ = command;
            format!(
                "slash command {command_name} is interactive-only. Start `eidolon` and run it there, or use `eidolon --resume SESSION.jsonl {command_name}` / `eidolon --resume {latest} {command_name}` when the command is marked [resume] in /help.",
                command_name = rest[0],
...
fn format_unknown_direct_slash_command(name: &str) -> String {
    ...
    message.push_str("\nRun `eidolon --help` for CLI usage, or start `eidolon` and use /help.");
    message
}
...
fn omc_compatibility_note_for_unknown_slash_command(name: &str) -> Option<&'static str> {
    name.starts_with("oh-my-claudecode:")
        .then_some(
            "Compatibility note: `/oh-my-claudecode:*` is a Claude Code/OMC plugin command. `eidolon` does not yet load plugin slash commands, Claude statusline stdin, or OMC session hooks.",
        )
}
...
fn render_resume_usage() -> String {
  ...
  Auto-save        .eidolon/sessions/<session-id>.{PRIMARY_SESSION_EXTENSION}
    ...
    match command {
        ...
        SlashCommand::Clear { confirm } => {
            ...
            Ok(ResumeCommandOutcome {
                session: cleared,
                message: Some(format!(
                    "Session cleared\n  Mode             resumed session reset\n  Previous session {previous_session_id}\n  Backup           {}\n  Resume previous  eidolon --resume {}\n  New session      {new_session_id}\n  Session file     {}",
                    backup_path.display(),
                    backup_path.display(),
        ...
        SlashCommand::Skills { args } => {
            if let SkillSlashDispatch::Invoke(_) = classify_skills_slash_command(args.as_deref()) {
                return Err(
                    "resumed /skills invocations are interactive-only; start `eidolon` and run `/skills <skill>` in the REPL".into(),
                );
            }
            let cwd = env::current_dir()?;
            Ok(ResumeCommandOutcome {
...
fn sessions_dir() -> Result<PathBuf, Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    let path = cwd.join(".eidolon").join("sessions");
...
fn format_missing_session_reference(reference: &str) -> String {
    format!(
        "session not found: {reference}\nHint: managed sessions live in .eidolon/sessions/. Try `{LATEST_SESSION_REFERENCE}` for the most recent session or `/session list` in the REPL."
    )
}

fn format_no_managed_sessions() -> String {
    format!(
        "no managed sessions found in .eidolon/sessions/\nStart `eidolon` to create a session, then rerun with `--resume {LATEST_SESSION_REFERENCE}`."
    )
}
...
fn render_repl_help() -> String {
    [
        ...
        "  Auto-save            .eidolon/sessions/<session-id>.jsonl".to_string(),
...
fn render_help_topic(topic: LocalHelpTopic) -> String {
  ...
  Usage            eidolon status
  Purpose          show the local workspace snapshot without entering the REPL
  Output           model, permissions, git state, config files, and sandbox status
  Related          /status · eidolon --resume latest /status"
            .to_string(),
        LocalHelpTopic::Sandbox => "Sandbox
  Usage            eidolon sandbox
  Purpose          inspect the resolved sandbox and isolation state for the current directory
  Output           namespace, network, filesystem, and fallback details
  Related          /sandbox · eidolon status"
            .to_string(),
        LocalHelpTopic::Doctor => "Doctor
  Usage            eidolon doctor
  Purpose          diagnose local auth, config, workspace, sandbox, and build metadata
  Output           local-only health report; no provider request or session resume required
  Related          /doctor · eidolon --resume latest /doctor"
            .to_string(),
    }
}
...
fn format_context_window_blocked_error(session_id: &str, error: &api::ApiError) -> String {
    ...
    lines.push(format!(
        "  Resume compact   eidolon --resume {session_id} /compact"
    ));
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    writeln!(out, "eidolon v{VERSION}")?;
    writeln!(out)?;
    writeln!(out, "Usage:")?;
    writeln!(
        out,
        "  eidolon [--model MODEL] [--allowedTools TOOL[,TOOL...]]"
    )?;
    writeln!(out, "      Start the interactive REPL")?;
    writeln!(
        out,
        "  eidolon [--model MODEL] [--output-format text|json] prompt TEXT"
    )?;
    writeln!(out, "      Send one prompt and exit")?;
    writeln!(
        out,
        "  eidolon [--model MODEL] [--output-format text|json] TEXT"
    )?;
    writeln!(out, "      Shorthand non-interactive prompt mode")?;
    writeln!(
        out,
        "  eidolon --resume [SESSION.jsonl|session-id|latest] [/status] [/compact] [...]"
    )?;
    writeln!(
        out,
        "      Inspect or maintain a saved session without entering the REPL"
    )?;
    writeln!(out, "  eidolon help")?;
    writeln!(out, "      Alias for --help")?;
    writeln!(out, "  eidolon version")?;
    writeln!(out, "      Alias for --version")?;
    writeln!(out, "  eidolon status")?;
    writeln!(
        out,
        "      Show the current local workspace status snapshot"
    )?;
    writeln!(out, "  eidolon sandbox")?;
    writeln!(out, "      Show the current sandbox isolation snapshot")?;
    writeln!(out, "  eidolon doctor")?;
    writeln!(
        out,
        "      Diagnose local auth, config, workspace, and sandbox health"
    )?;
    writeln!(out, "  eidolon bootstrap-plan")?;
    ...
    writeln!(
        out,
        "  REPL turns auto-save to .eidolon/sessions/<session-id>.{PRIMARY_SESSION_EXTENSION}"
    )?;
    writeln!(
        out,
    ...
    writeln!(out, "  eidolon --model claude-opus \"summarize this repo\"")?;
    writeln!(
        out,
        "  eidolon --output-format json prompt \"explain src/main.rs\""
    )?;
    writeln!(
        out,
        "  eidolon --allowedTools read,glob \"summarize Cargo.toml\""
    )?;
    writeln!(out, "  eidolon --resume {LATEST_SESSION_REFERENCE}")?;
    writeln!(
        out,
        "  eidolon --resume {LATEST_SESSION_REFERENCE} /status /diff /export notes.txt"
    )?;
    writeln!(out, "  eidolon agents")?;
    writeln!(out, "  eidolon mcp show my-server")?;
    writeln!(out, "  eidolon /skills")?;
    writeln!(out, "  eidolon doctor")?;
    writeln!(out, "  eidolon login")?;
    writeln!(out, "  eidolon init")?;
...
mod tests {
    ...
    fn context_window_preflight_errors_render_recovery_steps() {
        ...
        assert!(
            rendered.contains("Resume compact   eidolon --resume session-issue-32 /compact"),
            "{rendered}"
        );
        assert!(
    ...
    fn temp_dir() -> PathBuf {
        ...
        std::env::temp_dir().join(format!("eidolon-cli-{nanos}-{unique}"))
    }
    ...
    fn default_permission_mode_uses_project_config_when_env_is_unset() {
        ...
        std::fs::create_dir_all(cwd.join(".eidolon")).expect("project config dir should exist");
        std::fs::create_dir_all(&config_home).expect("config home should exist");
        std::fs::write(
            cwd.join(".eidolon").join("settings.json"),
            r#"{"permissionMode":"acceptEdits"}"#,
        )
        .expect("project config should write");

        let original_config_home = std::env::var("EIDOLON_CONFIG_HOME").ok();
        ...
        match original_config_home {
            Some(value) => std::env::set_var("EIDOLON_CONFIG_HOME", value),
            None => std::env::remove_var("EIDOLON_CONFIG_HOME"),
        }
    ...
    fn parses_direct_agents_mcp_and_skills_slash_commands() {
        ...
        assert!(error.contains("eidolon --resume SESSION.jsonl /status"));
    }
    ...
    fn rejects_unknown_options_with_helpful_guidance() {
        ...
        assert!(error.contains("eidolon --help"));
    }
    ...
    fn repl_help_includes_shared_commands_and_exit() {
        ...
        assert!(help.contains("Auto-save            .eidolon/sessions/<session-id>.jsonl"));
    ...
    fn init_help_mentions_direct_subcommand() {
        ...
        assert!(help.contains("eidolon help"));
        assert!(help.contains("eidolon version"));
        assert!(help.contains("eidolon status"));
        assert!(help.contains("eidolon sandbox"));
        assert!(help.contains("eidolon init"));
        assert!(help.contains("eidolon agents"));
        assert!(help.contains("eidolon mcp"));
        assert!(help.contains("eidolon skills"));
        assert!(help.contains("eidolon /skills"));
    }
...
UU conflicted.rs",
    ...
    fn help_mentions_jsonl_resume_examples() {
        ...
        print_help_to(&mut help).expect("help should render");
        let help = String::from_utf8(help).expect("help should be utf8");
        assert!(help.contains("eidolon --resume [SESSION.jsonl|session-id|latest]"));
        assert!(help.contains("Use `latest` with --resume, /resume, or /session switch"));
        assert!(help.contains("eidolon --resume latest"));
        assert!(help.contains("eidolon --resume latest /status /diff /export notes.txt"));
    }
    ...
    fn managed_sessions_default_to_jsonl_and_resolve_legacy_json() {
        ...
        let legacy_path = workspace.join(".eidolon/sessions/legacy.json");
    ...
    fn resume_usage_mentions_latest_shortcut() {
        ...
        assert!(usage.contains(".eidolon/sessions/<session-id>.jsonl"));
    ...
    fn temp_workspace(label: &str) -> PathBuf {
        let nanos = std::time::SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("eidolon-cli-{label}-{nanos}"))
    }
    ...
    fn ultraplan_progress_lines_include_phase_step_and_elapsed_status() {
        let snapshot = InternalPromptProgressState {
            ...
            detail: Some("reading rust/crates/eidolon-cli/src/main.rs".to_string()),
            saw_final_text: false,
        };
    ...
    fn describe_tool_progress_summarizes_known_tools() {
        ...
        assert!(
            describe_tool_progress("bash", r#"{"command":"cargo test -p eidolon-cli"}"#)
                .contains("cargo test -p eidolon-cli")
        );
        assert_eq!(
