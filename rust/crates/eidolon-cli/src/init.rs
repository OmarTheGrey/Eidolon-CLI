...
const STARTER_EIDOLON_JSON: &str = concat!(
    "{\n",
    "  \"permissions\": {\n",
    "    \"defaultMode\": \"dontAsk\"\n",
    "  }\n",
...
const GITIGNORE_COMMENT: &str = "# Eidolon local artifacts";
const GITIGNORE_ENTRIES: [&str; 2] = [".eidolon/settings.local.json", ".eidolon/sessions/"];
...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let eidolon_dir = cwd.join(".eidolon");
    artifacts.push(InitArtifact {
        name: ".eidolon/",
        status: ensure_dir(&eidolon_dir)?,
    });

    let eidolon_json = cwd.join(".eidolon.json");
    artifacts.push(InitArtifact {
        name: ".eidolon.json",
        status: write_file_if_missing(&eidolon_json, STARTER_EIDOLON_JSON)?,
    });
...
pub(crate) fn render_init_claude_md(cwd: &Path) -> String {
    ...
    lines.push("- Keep shared defaults in `.eidolon.json`; reserve `.eidolon/settings.local.json` for machine-local overrides.".to_string());
...
mod tests {
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert!(rendered.contains(".eidolon/"));
        assert!(rendered.contains(".eidolon.json"));
        assert!(rendered.contains("created"));
        assert!(rendered.contains(".gitignore       created"));
        assert!(rendered.contains("CLAUDE.md        created"));
        assert!(root.join(".eidolon").is_dir());
        assert!(root.join(".eidolon.json").is_file());
        assert!(root.join("CLAUDE.md").is_file());
        assert_eq!(
            fs::read_to_string(root.join(".eidolon.json")).expect("read eidolon json"),
            concat!(
                "{\n",
                "  \"permissions\": {\n",
                "    \"defaultMode\": \"dontAsk\"\n",
        ...
        assert!(gitignore.contains(".eidolon/settings.local.json"));
        assert!(gitignore.contains(".eidolon/sessions/"));
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        assert!(second_rendered.contains(".eidolon/"));
        assert!(second_rendered.contains(".eidolon.json"));
        ...
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
        assert_eq!(gitignore.matches(".eidolon/settings.local.json").count(), 1);
        assert_eq!(gitignore.matches(".eidolon/sessions/").count(), 1);
