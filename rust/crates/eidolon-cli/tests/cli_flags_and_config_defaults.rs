...
fn status_command_applies_model_and_permission_mode_flags() {
    ...
    // when
    let output = Command::new(env!("CARGO_BIN_EXE_eidolon-cli"))
...
fn slash_command_names_match_known_commands_and_suggest_nearby_unknown_ones() {
    ...
    // when
    let help_output = Command::new(env!("CARGO_BIN_EXE_eidolon-cli"))
        .current_dir(&temp_dir)
        .arg("/help")
        .output()
        .expect("eidolon should launch");
    let unknown_output = Command::new(env!("CARGO_BIN_EXE_eidolon-cli"))
        .current_dir(&temp_dir)
        .arg("/zstats")
        .output()
        .expect("eidolon should launch");
...
fn config_command_loads_defaults_from_standard_config_locations() {
    ...
    let config_home = temp_dir.join("home").join(".eidolon");
    fs::create_dir_all(temp_dir.join(".eidolon")).expect("project config dir should exist");
    ...
    fs::write(temp_dir.join(".eidolon.json"), r#"{"model":"sonnet"}"#)
        .expect("write project settings");
    fs::write(
        temp_dir.join(".eidolon").join("settings.local.json"),
        r#"{"model":"opus"}"#,
    )
    ...
    let output = command_in(&temp_dir)
        .env("EIDOLON_CONFIG_HOME", &config_home)
        .args([
            "--resume",
            session_path.to_str().expect("utf8 path"),
            "/config",
    ...
    assert!(stdout.contains(temp_dir.join(".eidolon.json").to_str().expect("utf8 path")));
    assert!(stdout.contains(
        temp_dir
            .join(".eidolon")
...
fn doctor_command_runs_as_a_local_shell_entrypoint() {
    ...
    let output = command_in(&temp_dir)
        ...
        .arg("doctor")
        .output()
        .expect("eidolon doctor should launch");
...
fn local_subcommand_help_does_not_fall_through_to_runtime_or_provider_calls() {
    ...
    assert!(doctor_stdout.contains("Usage            eidolon doctor"));
    ...
    assert!(status_stdout.contains("Usage            eidolon status"));
...
fn command_in(cwd: &Path) -> Command {
    let mut command = Command::new(env!("CARGO_BIN_EXE_eidolon-cli"));
