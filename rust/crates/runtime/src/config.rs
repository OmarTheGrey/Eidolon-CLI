...
pub const EIDOLON_SETTINGS_SCHEMA_NAME: &str = "SettingsSchema";
...
/// OAuth client configuration used by the main Eidolon runtime.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl ConfigLoader {
    ...
    pub fn discover(&self) -> Vec<ConfigEntry> {
        let user_legacy_path = self.config_home.parent().map_or_else(
            || PathBuf::from(".eidolon.json"),
            |parent| parent.join(".eidolon.json"),
        );
        vec![
            ConfigEntry {
            ...
            ConfigEntry {
                ...
                path: self.cwd.join(".eidolon.json"),
            },
            ConfigEntry {
                source: ConfigSource::Project,
                path: self.cwd.join(".eidolon").join("settings.json"),
            },
            ConfigEntry {
                source: ConfigSource::Local,
                path: self.cwd.join(".eidolon").join("settings.local.json"),
            },
        ]
    }
...
fn read_optional_json_object(
    ...
    let is_legacy_config = path.file_name().and_then(|name| name.to_str()) == Some(".eidolon.json");
...
mod tests {
    use super::{
        ...
        RuntimePluginConfig, EIDOLON_SETTINGS_SCHEMA_NAME,
    };
    use crate::json::JsonValue;
    ...
    fn rejects_non_object_settings_files() {
        ...
        let home = root.join("home").join(".eidolon");
    ...
    fn loads_and_merges_claude_code_config_files_by_precedence() {
        ...
        let cwd = root.join("project");
        let home = root.join("home").join(".eidolon");
        fs::create_dir_all(cwd.join(".eidolon")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");

        fs::write(
            home.parent().expect("home parent").join(".eidolon.json"),
        ...
        assert_eq!(EIDOLON_SETTINGS_SCHEMA_NAME, "SettingsSchema");
    ...
    fn rejects_invalid_hook_entries_before_merge() {
        ...
        let cwd = root.join("project");
        let home = root.join("home").join(".eidolon");
        let project_settings = cwd.join(".eidolon").join("settings.json");
        fs::create_dir_all(cwd.join(".eidolon")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");
