...
fn build_search_url(query: &str) -> Result<reqwest::Url, String> {
    if let Ok(base) = std::env::var("EIDOLON_WEB_SEARCH_BASE_URL") {
...
fn todo_store_path() -> Result<std::path::PathBuf, String> {
    if let Ok(path) = std::env::var("EIDOLON_TODO_STORE") {
        return Ok(std::path::PathBuf::from(path));
    }
    let cwd = std::env::current_dir().map_err(|error| error.to_string())?;
    Ok(cwd.join(".eidolon-todos.json"))
}
...
fn agent_store_dir() -> Result<std::path::PathBuf, String> {
    if let Ok(path) = std::env::var("EIDOLON_AGENT_STORE") {
        return Ok(std::path::PathBuf::from(path));
    }
    let cwd = std::env::current_dir().map_err(|error| error.to_string())?;
    if let Some(workspace_root) = cwd.ancestors().nth(2) {
        return Ok(workspace_root.join(".eidolon-agents"));
    }
    Ok(cwd.join(".eidolon-agents"))
}
...
fn normalize_subagent_type(subagent_type: Option<&str>) -> String {
    ...
    match canonical_tool_token(trimmed).as_str() {
        ...
        "eidolonguide" | "eidolonguideagent" | "guide" => String::from("eidolon-guide"),
...
fn config_file_for_scope(scope: ConfigScope) -> Result<PathBuf, String> {
    ...
    Ok(match scope {
        ...
        ConfigScope::Settings => cwd.join(".eidolon").join("settings.local.json"),
    })
}

fn config_home_dir() -> Result<PathBuf, String> {
    if let Ok(path) = std::env::var("EIDOLON_CONFIG_HOME") {
        return Ok(PathBuf::from(path));
    }
    let home = std::env::var("HOME").map_err(|_| String::from("HOME is not set"))?;
    Ok(PathBuf::from(home).join(".eidolon"))
}
...
mod tests {
    ...
    fn temp_path(name: &str) -> PathBuf {
        let unique = std::time::SystemTime::now()
            ...
            .expect("time")
            .as_nanos();
        std::env::temp_dir().join(format!("eidolon-tools-{unique}-{name}"))
    }
    ...
    fn web_search_extracts_and_filters_results() {
        ...
        std::env::remove_var("EIDOLON_WEB_SEARCH_BASE_URL");
    ...
    fn web_search_handles_generic_links_and_invalid_base_url() {
        ...
        std::env::set_var("EIDOLON_WEB_SEARCH_BASE_URL", "://bad-base-url");
    ...
    fn todo_write_persists_and_returns_previous_state() {
        ...
        std::env::set_var("EIDOLON_TODO_STORE", &path);
        ...
        .expect("TodoWrite should succeed");
        std::env::remove_var("EIDOLON_TODO_STORE");
    ...
    fn skill_resolves_project_local_skills_and_legacy_commands() {
        ...
        let skill_dir = root.join(".eidolon").join("skills").join("plan");
        let command_dir = root.join(".eidolon").join("commands");
        fs::create_dir_all(&skill_dir).expect("skill dir should exist");
    ...
    fn agent_persists_handoff_metadata() {
        ...
        std::env::set_var("EIDOLON_AGENT_STORE", &dir);
        ...
        std::env::remove_var("EIDOLON_AGENT_STORE");
  ...
  "nbformat_minor": 5
    ...
    fn config_reads_and_writes_supported_values() {
        ...
        std::fs::create_dir_all(home.join(".eidolon")).expect("home dir");
        std::fs::create_dir_all(cwd.join(".eidolon")).expect("cwd dir");
        std::fs::write(
            home.join(".eidolon").join("settings.json"),
            r#"{"verbose":false}"#,
        )
    ...
    fn enter_and_exit_plan_mode_round_trip_existing_local_override() {
        ...
        std::fs::create_dir_all(cwd.join(".eidolon")).expect("cwd dir");
        std::fs::write(
            cwd.join(".eidolon").join("settings.local.json"),
...
"#,
    ...
    fn run_task_packet_creates_packet_backed_task() {
        let result = run_task_packet(TaskPacket {
            ...
            repo: "eidolon-code-parity".to_string(),
        ...
        assert_eq!(output["task_packet"]["repo"], "eidolon-code-parity");
        assert_eq!(
