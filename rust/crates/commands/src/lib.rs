...
const SLASH_COMMAND_SPECS: &[SlashCommandSpec] = &[
    ...
    SlashCommandSpec {
        ...
        summary: "Manage Eidolon plugins",
        argument_hint: Some(
...
impl DefinitionSource {
    fn report_scope(self) -> DefinitionScope {
        match self {
            Self::ProjectEidolon | Self::ProjectCodex | Self::ProjectClaude => {
                DefinitionScope::Project
            }
            Self::UserEidolonConfigHome | Self::UserCodexHome => DefinitionScope::UserConfigHome,
            Self::UserEidolon | Self::UserCodex | Self::UserClaude => DefinitionScope::UserHome,
        }
    }
...
fn discover_definition_roots(cwd: &Path, leaf: &str) -> Vec<(DefinitionSource, PathBuf)> {
    ...
    for ancestor in cwd.ancestors() {
        push_unique_root(
            &mut roots,
            DefinitionSource::ProjectEidolon,
            ancestor.join(".eidolon").join(leaf),
        );
        push_unique_root(
            &mut roots,
    ...
    if let Ok(eidolon_config_home) = env::var("EIDOLON_CONFIG_HOME") {
        push_unique_root(
            &mut roots,
            DefinitionSource::UserEidolonConfigHome,
            PathBuf::from(eidolon_config_home).join(leaf),
        );
    }
...
fn discover_skill_roots(cwd: &Path) -> Vec<SkillRoot> {
    ...
    for ancestor in cwd.ancestors() {
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectEidolon,
            ancestor.join(".eidolon").join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectEidolon,
            ancestor.join(".eidolon").join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
        push_unique_skill_root(
            &mut roots,
    ...
    if let Ok(eidolon_config_home) = env::var("EIDOLON_CONFIG_HOME") {
        let eidolon_config_home = PathBuf::from(eidolon_config_home);
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserEidolonConfigHome,
            eidolon_config_home.join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserEidolonConfigHome,
            eidolon_config_home.join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
    }
    ...
    if let Some(home) = env::var_os("HOME") {
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserEidolon,
            home.join(".eidolon").join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
        push_unique_skill_root(
            &mut roots,
...
fn default_skill_install_root() -> std::io::Result<PathBuf> {
    if let Ok(eidolon_config_home) = env::var("EIDOLON_CONFIG_HOME") {
        return Ok(PathBuf::from(eidolon_config_home).join("skills"));
    }
    if let Ok(codex_home) = env::var("CODEX_HOME") {
        return Ok(PathBuf::from(codex_home).join("skills"));
    }
    if let Some(home) = env::var_os("HOME") {
        return Ok(PathBuf::from(home).join(".eidolon").join("skills"));
    }
    Err(std::io::Error::new(
        std::io::ErrorKind::NotFound,
        "unable to resolve a skills install root; set EIDOLON_CONFIG_HOME or HOME",
    ))
}
...
fn render_agents_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "direct_cli": "eidolon agents [list|help]",
...
fn render_skills_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       eidolon skills [list|install <path>|help|<skill> [args]]".to_string(),
        "  Invoke           /skills help overview -> $help overview".to_string(),
        "  Install root     $EIDOLON_CONFIG_HOME/skills or ~/.eidolon/skills".to_string(),
        "  Sources          .eidolon/skills, .omc/skills, .agents/skills, .codex/skills, .claude/skills, ~/.eidolon/skills, ~/.omc/skills, ~/.claude/skills/omc-learned, ~/.codex/skills, ~/.claude/skills, legacy /commands".to_string(),
    ];
    if let Some(args) = unexpected {
        lines.push(format!("  Unexpected       {args}"));
    }
...
fn render_skills_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "aliases": ["/skill"],
            "direct_cli": "eidolon skills [list|install <path>|help|<skill> [args]]",
            "invoke": "/skills help overview -> $help overview",
            "install_root": "$EIDOLON_CONFIG_HOME/skills or ~/.eidolon/skills",
            "sources": [
                ".eidolon/skills",
                ".omc/skills",
                ".agents/skills",
...
fn render_mcp_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       eidolon mcp [list|show <server>|help]".to_string(),
        "  Sources          .eidolon/settings.json, .eidolon/settings.local.json".to_string(),
    ];
    if let Some(args) = unexpected {
        lines.push(format!("  Unexpected       {args}"));
    }
...
fn render_mcp_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "direct_cli": "eidolon mcp [list|show <server>|help]",
            "sources": [".eidolon/settings.json", ".eidolon/settings.local.json"],
        },
        "unexpected": unexpected,
    })
}
...
fn definition_source_id(source: DefinitionSource) -> &'static str {
    match source {
        DefinitionSource::ProjectEidolon
        | DefinitionSource::ProjectCodex
        | DefinitionSource::ProjectClaude => "project_eidolon",
...
mod tests {
    ...
    fn renders_per_command_help_detail() {
        ...
        assert!(help.contains("Summary          Manage Eidolon plugins"));
        assert!(help.contains("Aliases          /plugins, /marketplace"));
    ...
    fn renders_agents_reports_as_json() {
        ...
        assert_eq!(report["agents"][2]["shadowed_by"]["id"], "project_eidolon");

        let help = handle_agents_slash_command_json(Some("help"), &workspace).expect("agents help");
        assert_eq!(help["kind"], "agents");
        assert_eq!(help["action"], "help");
        assert_eq!(help["usage"]["direct_cli"], "eidolon agents [list|help]");
    ...
    fn resolves_project_skills_and_legacy_commands_from_shared_registry() {
        ...
        let project_skills = workspace.join(".eidolon").join("skills");
        let legacy_commands = workspace.join(".eidolon").join("commands");
    ...
    fn renders_skills_reports_as_json() {
        ...
        assert_eq!(report["skills"][0]["source"]["id"], "project_eidolon");
        assert_eq!(report["skills"][1]["name"], "deploy");
        assert_eq!(report["skills"][1]["origin"]["id"], "legacy_commands_dir");
        assert_eq!(report["skills"][3]["shadowed_by"]["id"], "project_eidolon");
        ...
        assert_eq!(
            ...
            "eidolon skills [list|install <path>|help|<skill> [args]]"
        );
    ...
    fn mcp_usage_supports_help_and_unexpected_args() {
        ...
        assert!(help.contains("Direct CLI       eidolon mcp [list|show <server>|help]"));
    ...
    fn renders_mcp_reports_from_loaded_config() {
        ...
        fs::create_dir_all(workspace.join(".eidolon")).expect("workspace config dir");
        fs::create_dir_all(&config_home).expect("config home");
        fs::write(
            workspace.join(".eidolon").join("settings.json"),
            r#"{
              "mcpServers": {
                "alpha": {
                  "command": "uvx",
        ...
        fs::write(
            workspace.join(".eidolon").join("settings.local.json"),
            r#"{
              "mcpServers": {
                "remote": {
                  "type": "ws",
    ...
    fn renders_mcp_reports_as_json() {
        ...
        assert_eq!(help["action"], "help");
        assert_eq!(help["usage"]["sources"][0], ".eidolon/settings.json");
